...
vorpal.command('rgb [rgb...]', 'Classify a new color').action((args, cb) => {
  ...
  console.log(chalk.rgb(r, g, b).bgKeyword('white')('sample color: #####'));
  _(net.run(args.rgb))
    .toPairs()
    .map(([key, val]) => [key, val < 0.005 ? 0 : _.round(val * 100, 2)])
    .sortBy(([_, val]) => val)
    .reverse()
    .map(([key, val]) => {
      console.log(chalk.white.bgBlack(`${key}: ${val}`));
    })
    .value();
  cb();
});
...
vorpal
  .command('train', 'Train a new neural net based of color file')
  .action((args, cb) => {
